...
class MTDI_Parser {
	function parse( $file ) {
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '<p><strong>' . esc_html__( 'There was an error when reading this WXR file', 'mysterythemes-demo-importer' ) . '</strong><br />';

			echo esc_html__( 'Details are shown above. The importer will now try again with a different parser...', 'mysterythemes-demo-importer' ) . '</p>';
...
class MTDI_Parser_SimpleXML {
	...
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'mysterythemes-demo-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'mysterythemes-demo-importer' ) );
		}
