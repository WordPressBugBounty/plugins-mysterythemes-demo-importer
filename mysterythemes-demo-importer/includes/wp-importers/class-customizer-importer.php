...
class MTDI_Customizer_Importer {
	...
	public static function import( $import_file, $demo_id, $demo_data ) {
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error( 'mtdi_import_data_error', esc_html__( 'The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'mysterythemes-demo-importer' ) );
		}

		if ( is_child_theme() ) {
			if ( strpos( $demo_data['parent_slug'], $template ) === false ) {
				return new WP_Error( 'mtdi_import_wrong_theme', esc_html__( 'The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'mysterythemes-demo-importer' ) );
			}
		} else {
